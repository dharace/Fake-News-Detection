...
class TfidfEmbeddingVectorizer(object):
    ...
    def __init__(self, train_news):
        ...
        self.train_count = self.countV.fit_transform(self.train_news['Statement'].values)
        ...
        #tf-idf 
        self.tfidfV = TfidfTransformer()
        self.train_tfidf = self.tfidfV.fit_transform(self.train_count)

        #bag of words - with n-grams
        #countV_ngram = CountVectorizer(ngram_range=(1,3),stop_words='english')
        #tfidf_ngram  = TfidfTransformer(use_idf=True,smooth_idf=True)

        self.tfidf_ngram = TfidfVectorizer(stop_words='english',ngram_range=(1,4),use_idf=True,smooth_idf=True)

        #POS Tagging
        self.tagged_sentences = nltk.corpus.treebank.tagged_sents()

        self.cutoff = int(.75 * len(self.tagged_sentences))
        self.training_sentences = self.train_news['Statement']
 
        print(self.training_sentences)
    

    #print training doc term matrix
    #we have matrix of size of (10240, 12196) by calling below
    def get_countVectorizer_stats(self):
        #vocab size
        self.train_count.shape

        #check vocabulary using below command
        print(self.countV.vocabulary_)

        #get feature names
        print(self.countV.get_feature_names()[:25])


    def get_tfidf_stats(self):
        self.train_tfidf.shape
        #get train data feature names 
        print(self.train_tfidf.A[:10])
